...
- モデル自動取得はプロバイダー別APIを呼び、API由来モデルを置換しつつカスタムモデルを保持する。
...
- Azureは`resourceName/useDeploymentBasedUrls`を含む`AzureProviderConfig`に従う。
- モデル刷新はAPIモデルを全置換し、`source=custom`のモデルは保持する。
