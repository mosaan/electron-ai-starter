...
- プロキシ/証明書/カスタムfetch (`proxy.test.ts`, `certificate.test.ts`, `fetch.test.ts`) は **WIP**。ログ/DB初期化の順序依存によるインフラ課題で失敗することがREADMEで報告されている。
- DBはSQLite（better-sqlite3）を実ファイルで使用。`tests/setup.ts` で `MAIN_VITE_USER_DATA_PATH` を `tests/tmp` 配下へ強制設定。
...
## 実行手順（現状）
- Backend全体（WIPを含む）: `pnpm run test:backend`  
  - 既知の失敗を避けるには、動作確認済みのファイルのみを指定して実行する:  
    `npx vitest run tests/backend/database.test.ts tests/backend/utils.test.ts`  
    （他のテストは依存初期化の問題が解決していない可能性あり）
- Renderer: `pnpm run test:renderer`（watch/UIオプションは`test:renderer:watch`/`test:renderer:ui`）。happy-dom環境でヘッドレス実行。

## 既知の課題とリスク
- **依存初期化の順序依存**: ロガー/DB初期化が早期に走るため、モックより先に本物のパス解決が行われテストが失敗（proxy/cert/fetch系）。→ ロガーの遅延初期化やDB接続のDIが必要。
