...
- **作成日**: 2025-11-18
- **バージョン**: 2.0（全面改訂）
- **対象ユースケース**: UC-06A 〜 UC-09C（13個のユースケース）
...
**v2.0 (2025-11-18)**
- コードベース調査に基づき全面改訂
- 10個の新規ユースケースを追加（UC-06A, UC-06B, UC-07A, UC-07B, UC-08A, UC-08B, UC-08C, UC-09A, UC-09B, UC-09C）
- 要件数を58から115に拡充
- 実装済み機能を完全に文書化

**v1.0 (初版)**
- 基本的なチャット機能のみ（UC-06〜UC-09、4個のユースケース）
...
When モデル選択が復元された, the システム shall 復元された選択が現在の設定で有効かUC-06Aの要件7に従って検証する
...
エラーハンドリングの詳細は、UC-09A, UC-09B, UC-09Cを参照してください。

**26. ネットワークエラーの処理**

If メッセージ送信中にネットワークエラーが発生した, the システム shall UC-09Bのエラー分類に従ってエラーを処理する

**27. 認証エラーの処理**

If メッセージ送信中に認証エラー（401、403）が発生した, the システム shall UC-09Bのエラー分類に従ってエラーを処理する

**28. レート制限エラーの処理**

If メッセージ送信中にレート制限エラー（429）が発生した, the システム shall UC-09Bのエラー分類に従ってエラーを処理する
...
メッセージ送信前にシステムがトークン制限をチェックし、必要に応じて会話履歴を自動圧縮します。

### 4.2 事前条件

- The システム shall チャットセッションIDが存在する
- The システム shall 圧縮設定のautoCompressがtrueである

### 4.3 事後条件

- If 圧縮が必要, the システム shall 会話履歴が圧縮されている
...
When ユーザーがメッセージを送信した, the システム shall 現在のセッションの全メッセージの合計トークン数を計算する

**30. 圧縮閾値の確認**

When トークン数が計算された, the システム shall モデルのコンテキスト制限の80%を超えているか確認する

**31. 圧縮設定の尊重**

If 圧縮設定のautoCompressがfalse, the システム shall 自動圧縮をスキップする

#### 4.4.2 自動圧縮の実行

**32. 圧縮の開始**

When トークン数が圧縮閾値を超えた, the システム shall 自動的に会話履歴の圧縮を開始する

**33. 圧縮対象の選択**

When 圧縮が開始された, the システム shall 最も古いメッセージ群（最新の10メッセージを除く）を圧縮対象とする

**34. 圧縮結果のログ記録**

When 圧縮が完了した, the システム shall 圧縮前後のトークン数、圧縮されたメッセージ数をログに記録する

**35. 圧縮失敗時の継続**

If 圧縮中にエラーが発生した, the システム shall エラーをログに記録し、メッセージ送信を継続する（圧縮失敗でメッセージ送信を中断しない）
...
When Backendからaiツール呼び出しイベントを受信した, the システム shall ツールカードをチャット履歴に表示する（UC-09参照）

**54. ツール結果イベントの処理**

When Backendからaiツール結果イベントを受信した, the システム shall 対応するツールカードに結果を表示する（UC-09参照）
...
When AIからの応答が完了した, the システム shall UC-07Bの要件40-44に従ってデータベースに保存する
...
If compressionSummariesが存在する, the システム shall UC-08Cの要件に従って圧縮マーカーをメッセージ履歴に挿入する
...
When ストリーミングが中断された, the システム shall 中断時点までの部分応答をUC-07Bの要件に従ってデータベースに保存する
...
- The システム shall UC-09Aによってメッセージが入力フィールドに復元されている
